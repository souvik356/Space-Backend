...
export const registerController = async (req, res) => {
  try {
    ...
    if (!validator.isStrongPassword(passwordHash)) {
      return res.status(400).json({
        message:
          "Please provide valid password inlcude special case and start with uppercase",
        error: true,
        succes: false,
      });
    }
...
export const getLoggedInUserDetail = async (req, res) => {
  try {
    const loggedInUser = req.user;
    const isUserInDb = await UserModel.findById(loggedInUser._id).populate({
      path: "spaces",
      select: "spaceName projects",
      populate: {
        path: "projects",
        select: "projectName", // Add fields you want from Project model
      },
    });
    if (!isUserInDb) {
      return res.status(400).json({
